...
async function upsertChannel(env, body) {
  assertDb(env);
  const project = await requireProject(env, body.projectSlug);
  const node = body.nodeSlug
    ? await ensureNodeForChannel(env, project, {
        slug: body.nodeSlug,
...
async function issueAgentToken(env, body) {
  ...
  await env.DB.prepare(
    ...
    .bind(id, project.id, name, tokenHash, now)
    .run();
...
async function runScheduledSweep(env, scheduledTime = Date.now(), options = {}) {
  ...
  const nodesQuery = scopedProject
    ? env.DB.prepare(
        `SELECT id, project_id, slug, expected_heartbeat_sec, last_heartbeat_at
         FROM nodes
         WHERE project_id = ?1`,
      ).bind(scopedProject.id)
    : env.DB.prepare(
        `SELECT id, project_id, slug, expected_heartbeat_sec, last_heartbeat_at
         FROM nodes`,
      );
  const channelsQuery = scopedProject
    ? env.DB.prepare(
